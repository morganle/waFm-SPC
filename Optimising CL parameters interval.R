...
data =read.csv("PhaseII_magnitudes_lambda_pt7interval.csv",header = FALSE)  # G=100 macro reps W=1000 contiguous windows waFm stats taking in 0th to 2500th mags
data2=read.csv("PhaseII_magnitudes_lambda_pt7interval.csv",header=FALSE)    # G=100 macro reps W=1200 contiguous windows
...
uniroot(find_param,W=W,lower=4,upper=6)  # searches for CL parameter 
